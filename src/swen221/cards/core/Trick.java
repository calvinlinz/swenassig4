...
public class Trick {
	...
	public void play(Player p, Card c) throws IllegalMove {
		...
		if(getCardPlayed(getLeadPlayer()) != null){
			Card leader = getCardPlayed(getLeadPlayer());
			if(!c.suit().equals(leader.suit())){
				for(Card card : p.getHand()) {
					if(card.suit().equals(leader.suit())) {
						throw new IllegalMove("Illegal incorrect suit");
					}
				}
			}
		...
		if(getNextToPlay() != p.getDirection()) {
			throw new IllegalMove("ILLEGAL MOVE not ur turn ");
		}
		
		for(int i=0; i < p.getHand().size();i++) {
			if(!p.getHand().contains(c)) {
				throw new IllegalMove("Illegal Move does not have card in hand");
			}
		}
