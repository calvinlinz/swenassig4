...
public abstract class AbstractCardGame implements CardGame {
	...
	public void helper(Map<Player.Direction, Player> players, Map<Player.Direction, Integer> tricks,Map<Player.Direction, Integer> scores) {
		for(Player.Direction dir : players.keySet()) {
			this.players.put(dir,players.get(dir).clone());
		}
		for(Player.Direction dir : tricks.keySet()) {
			this.tricks.put(dir,tricks.get(dir));
		}
		for(Player.Direction dir : scores.keySet()) {
			this.scores.put(dir,scores.get(dir));
		}
